...
<ListItem
  ...
  sx={{
    ...
    borderRadius: '26px', // Добавляем скругление углов
    mb: 2, // Добавляем отступ снизу для разделения сообщений
    maxWidth: '80%', // Устанавливаем максимальную ширину сообщения
    alignSelf: messageRole === 'user' ? 'flex-end' : 'flex-start', // Выравниваем сообщения пользователя справа
    marginLeft: messageRole === 'user' ? 'auto' : '16px', // Добавляем отступ от края для пользователя
    marginRight: messageRole === 'user' ? '16px' : 'auto', // Добавляем отступ от края для ассистента
    ...
    ...(isUserStarred && {
      ...
      boxShadow: 'lg',
      borderRadius: '16px', // Изменяем на скругленные углы
      zIndex: 1,
    }),
    '&:hover > button': { opacity: 1 },
    display: 'inline-block', // Изменяем display на inline-block для автоматической ширины
...
>
      ...
      <Box sx={{
        ...
        {/* Avatar (Persona) */}
        {showAvatar && (
          <Box sx={{ ...personaSx, borderRadius: '16px' }}>
